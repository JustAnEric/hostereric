...
class Node {
            ...
            setTimeout(function() {
                if (totalTimesPinged < times) {
                    if(!this.pingInUse) {
                        ...
                        this.pingInUse = true;
                        this.ip = "hostereric.herokuapp.com/NodeCDN/pan_tool_FILL0_wght400_GRAD0_opsz48.png";
                    
                        this.img = new Image();
                    
                        this.img.onload = function() { console.warn(`Response from our servers at ${ip} - ${new Date().getTime()}`); };
                        this.img.onerror = function() { console.error(`No response from our servers at ${ip}`); };
                    
                        this.img.src = "https://" + ip;
                    }
                    this.pingInUse = false;
                    totalTimesPinged += 1;
                }
            });
        }, 1000)
    ...
    makeFunction(code, executorOptions={}) {
        function make() {
            function exec(access) {
                if (access == "safe") {
                    return code().catch(function(err) { console.error(`[NODECDN] ${err}`) });
                } else { return code().catch(function(err) { console.error(`[NODECDN] [NON-SECURE] ${err}`) }); }
            }

            return exec;
        }
        if (!executorOptions['exec']) return code && make;
        if (executorOptions['exec']) return code() && make;
    }
    safeExec(func) {
      if (!func) {
    ...
    async request(method="GET", url, callback, headers={}, body="") {
        await fetch(url, {
            method: method,
            callback: callback,
            headers: headers,
    ...
    findElementInElement(element, tag) {
